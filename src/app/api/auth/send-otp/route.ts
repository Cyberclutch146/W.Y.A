...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      console.error('[send-otp] Email send failed:', mailErr.message, mailErr.code || '', mailErr.responseCode || '');
      return NextResponse.json({ error: `Email delivery failed: ${mailErr.code || mailErr.message}` }, { status: 500 });
    }
