...
export default function StepMedia({
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try { const url = await uploadImage(file, 'campaigns'); setImage(url); }
  ...
  return (
    <div>
      ...
      <div style={sectionStyle}>
        <div className="flex items-center justify-between mb-4">
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 transition-all disabled:opacity-50"
            style={{
              borderRadius: '4px',
        ...
        <div className="relative">
          <textarea
            ...
            placeholder="Describe the goal, format, and vibe of your event..."
            rows={6}
          ...
          />
          {description && (
            <span className="absolute bottom-3 right-3 text-[10px] tabular-nums" style={{ color: 'var(--cp-text-3)' }}>
              {description.length}
            </span>
          )}
        </div>
        {description && (
          <p className="text-xs mt-2 font-semibold" style={{ color: 'var(--cp-secondary)' }}>
            AI-drafted — feel free to edit
          </p>
        )}
      </div>
      ...
      <div style={sectionStyle}>
        <div className="flex items-center justify-between mb-4">
          ...
          <button
            ...
            style={{
              borderRadius: '4px',
              background: 'hsl(from var(--cp-cyan) h s l / 0.08)',
              color: 'var(--cp-cyan)',
              border: '1px solid hsl(from var(--cp-cyan) h s l / 0.25)',
            }}
          >
        ...
        <div className="flex items-center gap-3">
          <label
            className={`flex-1 flex items-center justify-center gap-2 py-5 text-sm font-semibold cursor-pointer transition-all ${generatingImage ? 'opacity-50 cursor-not-allowed' : 'hover:opacity-80'}`}
            style={{
          ...
          >
            ...
            <input type="file" accept="image/*" className="hidden"
              disabled={uploadingImage || generatingImage} onChange={handleImageUpload} />
          </label>

          {image && (
            <div className="relative w-20 h-20 overflow-hidden shrink-0 group"
              style={{ border: '1px solid var(--cp-border)', borderRadius: 0 }}>
        ...
        <p className="text-xs mt-2" style={{ color: 'var(--cp-text-3)' }}>Optional. A default image will be used if left blank.</p>
      </div>
      ...
      <div>
        ...
        >
          <span className="text-[10px] font-bold uppercase tracking-widest" style={{ color: 'var(--cp-text-3)' }}>
            Resource Needs <span className="ml-1 normal-case font-normal" style={{ color: 'var(--cp-text-3)' }}>(optional)</span>
          </span>
          {needsExpanded
            ? <ChevronUp size={14} style={{ color: 'var(--cp-text-3)' }} />
            : <ChevronDown size={14} style={{ color: 'var(--cp-text-3)' }} />
          }
        </button>
            ...
            >
              <div className="space-y-5 pt-5">
                ...
                <div className="flex items-center gap-4">
                  <input type="checkbox" id="needFunds" checked={needFunds}
                    onChange={e => setNeedFunds(e.target.checked)}
                    className="w-4 h-4 cursor-pointer" style={{ accentColor: 'var(--cp-primary)' }} />
                  <label htmlFor="needFunds" className="text-sm font-semibold cursor-pointer flex-1">Funding</label>
                  {needFunds && (
                    <input type="number" value={fundGoal}
                      onChange={e => setFundGoal(Number(e.target.value))}
                      placeholder="Goal ($)" className="input-base w-32"
                      style={{ padding: '0.5rem 0.75rem', fontSize: '0.8125rem', borderRadius: 0 }} />
                  )}
                </div>
                ...
                <div className="flex items-center gap-4">
                  ...
                  {needVols && (
                    <input type="number" value={volGoal}
                      ...
                      placeholder="Count" className="input-base w-32"
                      style={{ padding: '0.5rem 0.75rem', fontSize: '0.8125rem', borderRadius: 0 }} />
                  )}
                </div>
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-4">
                    <input type="checkbox" id="needGoods" checked={needGoods}
                      onChange={e => setNeedGoods(e.target.checked)}
                      className="w-4 h-4 cursor-pointer" style={{ accentColor: 'var(--cp-primary)' }} />
                    <label htmlFor="needGoods" className="text-sm font-semibold cursor-pointer">Goods / Items</label>
                  </div>
                  {needGoods && (
                    <div className="pl-8 space-y-3">
                      <div className="flex gap-2">
                        <input
                          type="text" value={goodsItem}
                          ...
                          style={{ padding: '0.5rem 0.875rem', fontSize: '0.8125rem', borderRadius: 0 }}
                        />
                        <button type="button"
                          onClick={() => { if (goodsItem.trim()) { setGoodsList([...goodsList, goodsItem.trim()]); setGoodsItem(''); } }}
                          className="btn-primary px-4"
                          style={{ borderRadius: '4px', padding: '0.5rem 0.875rem', fontSize: '0.8125rem' }}>
                          <Plus size={14} />
                        </button>
                      </div>
                      {goodsList.length > 0 && (
                        <ul className="space-y-1.5">
                          {goodsList.map((item, i) => (
                            <li key={i}
                              className="flex justify-between items-center px-3 py-2 text-sm font-medium"
                              style={{ border: '1px solid var(--cp-border)', borderRadius: 0 }}>
                              <span>{item}</span>
                              <button type="button" onClick={() => setGoodsList(goodsList.filter((_, j) => j !== i))}
                                className="w-6 h-6 flex items-center justify-center transition-all hover:opacity-80"
